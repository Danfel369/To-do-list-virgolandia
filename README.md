# To-do-list-virgolandia
- El To-Do List Virgolandia contará con 3 roles: 
    1- empleado 
    2- líder 
    3- sublíder

# REQUERIMIENTOS GENERALES
- Como usuario quiero poder registrarme e iniciar sesión en la plataforma.
- Como empleado individual quiero poder crear tareas, editarlas o eliminarlas.
- Como empleado individual quiero poder establecer una fecha límite para el cumplimiento de una tarea.
- Como empleado individual quiero que me llegue un recordatorio (Notificaciones al correo) con las fechas límites que establezca en cada tarea.
- Como empleado individual quiero agregar categorías de tareas y filtrarlas por categorías.
- Como líder quiero poder crear equipos y asignar tareas personalizadas a otros empleados miembros de mi equipo.
- Como líder quiero poder asignar sublíderes en caso de distintas áreas de trabajo, para que los sublíderes puedan manejar también a los miembros del equipo y asignarles tareas.
- Como miembro de un equipo quiero poder realizar peticiones al líder para poder ser sublíder de un equipo.
- La página web debe ser responsive y lograr estándares mínimos de accesibilidad (lectores de pantalla, colores bien contrastados, mantener coherencia en el desarrollo de la página, etc.).

# PRODUCTO MÍNIMO VIABLE
Con la finalidad de cumplir con un MVP para satisfacer la necesidad más básica del usuario, nuestro objetivo principal es cumplir con:
- Como usuario quiero poder registrarme e iniciar sesión en la plataforma.
- Como empleado individual quiero poder crear tareas, editarlas y eliminarlas.

# SECCIÓN "MI PERFIL"
Con el objetivo de que el usuario pueda modificar sus credenciales registradas en la plataforma, se brindará una sección de "Mi Perfil" para que este pueda modificar ciertos valores permitidos:
    - Contraseña.
    - Foto de perfil.
    - Correo electrónico.

# TECNOLOGÍAS
- FRONTEND: JavaScript, HTML, CSS y React.
- BACKEND: Java y SpringBoot.
- BASE DE DATOS: MongoDB.